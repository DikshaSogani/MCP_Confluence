...
#             return text.strip()
#         else:
#             print("  [PDF] PyMuPDF returned empty text — trying pdfplumber")
#     except ImportError:
#         print("  [PDF] PyMuPDF (fitz) not installed — trying pdfplumber")
...
#             return result
#     except ImportError:
#         msg = "[PDF extraction failed: neither PyMuPDF nor pdfplumber is installed. Run: pip install pymupdf pdfplumber]"
...
# # ── AUTO Image describer via Groq vision ───────────────────────
...
#                         "Be thorough — your description is the only way the user can understand this image."
#                     )},
#                 ]}],
#                 "max_tokens": 600,
#             },
...
# # ── AUTO attachment extractor (replaces content_extractor.py) ──

# def _auto_extract_attachments(page_id: str) -> str:
#     """
#     AUTO: Downloads and processes ALL attachments on a page without asking.
#     PDF   -> full text via PyMuPDF / pdfplumber
#     Image -> detailed description via Groq vision
#     Other -> filename + type noted
#     """
#     try:
...
#     results = data.get("results", [])
#     if not results:
#         print(f"  [AUTO] No attachments found on page {page_id}")
#         return ""
...
#         print(f"  [AUTO] Processing: '{title}' | mime: {mime} | url: {dl_url}")

#         if not dl_url:
#             print(f"  [AUTO] Skipping '{title}' — no download URL")
#             continue
...
#             print(f"  [AUTO] PDF '{title}' added to context ({len(text)} chars, capped at 6000)")
...
#     """
#     AUTO: Finds all links in the page HTML and fetches their content.
#     Internal Confluence links -> fetches that page's text
#     External URLs             -> fetches via BeautifulSoup
#     """
...
#         if CONFLUENCE_URL in url and "/pages/" in url:
...
#                     sections.append(f"[Linked Confluence Page: {inner.get('title', url)}]\n{inner_text[:2000]}")
#                     save_url(url, inner.get("title", url), "confluence", SPACE_KEY)
#                     continue

#         content = _fetch_url_text(url, max_chars=1500)
#         sections.append(f"[Linked URL: {url}]\n{content}")
#         save_url(url, url, "external")

#     return "\n\n".join(sections)


# # ── Main context builder — AUTO runs everything ────────────────

# def build_page_context(page: dict, follow_links: bool = True) -> str:
#     """
#     Builds full context from a Confluence page.
#     AUTOMATICALLY extracts — no need to ask separately:
#       1. Page body text
#       2. ALL PDF attachments  -> full text
#       3. ALL image attachments -> Groq vision description
#       4. ALL hyperlinks        -> fetched content
#     """
#     page_id   = page["id"]
#     title     = page["title"]
...
#     sections += (["", "--- Attachments (PDF text + image descriptions) ---", att_text]
...
#         sections += (["", "--- Linked Content (auto-fetched) ---", url_text]
#                      if url_text else ["", "--- Linked URLs: none ---"])

#     full = "\n".join(sections)
#     print(f"  [AUTO] Context ready: {len(full)} chars")
#     print(f"  [AUTO] Breakdown — page body: {len(body_text)} | attachments: {len(att_text)} | links: {len(url_text) if follow_links else 0}")
#     return full


# # ── Public API called by assistant.py ─────────────────────────
